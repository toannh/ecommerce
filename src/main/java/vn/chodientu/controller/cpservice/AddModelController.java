 ...
 * @author thunt
 */
@Controller("cpAddModelService")
@RequestMapping(value = "/cpservice/addmodel")
public class AddModelController extends BaseRest {
    ...
    /**
     * Thêm mới thông tin cơ bản của model
     *
     * @param form
     * @return
    ...
    @ResponseBody
    public Response add(@RequestBody ModelForm form) throws Exception {
        String id = "test";
        ...
        return modelService.add(model, id);
    }
    /**
     * Sửa model
     * @param form
    ...
    public Response edit(@RequestBody ModelForm form) throws Exception {
        if (viewer.getAdministrator() == null) {
            return new Response(false, "Bạn cần đăng nhập lại để thực hiện chức năng này", new HashMap<String, String>());
        }
        Model model = new Model();
        model.setCategoryId(form.getCategoryId());
        ...
        return modelService.edit(model, viewer.getAdministrator().getId());
    }
    ...
    @ResponseBody
    public Response successadd(@RequestBody ModelLog modelLog) throws Exception {

        String idUser = viewer.getAdministrator().getId();
        try {
            modelService.submitAdd(modelLog.getModelId(), idUser, modelLog.getMessage());
        } catch (Exception e) {
            return new Response(false, e.getMessage());
        }

        return new Response(true, "Đã thêm thành công vào Log");
    }
