var addmodel = {};
addmodel.init = function() {
    //vẽ breadcrumb
    layout.breadcrumb([
        ["Trang chủ", "/cp/index.html"],
        ["Quản trị model", "/cp/model.html"],
        ["Danh sách model đã thêm"]
    ]);
};
addmodel.add = function() {
    popup.open('popup-add-model', 'Thêm mới thông tin cơ bản model', template('/cp/tpl/model/add.tpl', {category: category}), [
        {
            ...
            fn: function() {
                ajaxSubmit({
                    service: '/cpservice/addmodel/add.json',
                    id: 'model-add-form',
                    contentType: 'json',
                    done: function(resp) {
                        if (resp.success) {
...
addmodel.addImage = function() {
    ajaxUpload({
        service: '/cpservice/addmodel/addimage.json',
        id: 'image-model',
        done: function(resp) {
            if (resp.success) {
                var imghtml = '';
                for (var i in resp.data) {
                    imghtml += '<li style="float: left; list-style: none; padding: 0px 5px; margin: 0px 5px; height: 100px; width: 100px; margin-bottom: 50px;" for="' + i + '">\
                                    <a style="cursor:pointer" >\
                                        ...
                                        <div align="center" title="Xóa ảnh này" onclick="addmodel.deleteImage(\'' + resp.data[i] + '\', ' + i + ')">\
...
addmodel.deleteImage = function(name, i) {
    popup.confirm('Bạn có chắc chắn muốn xóa?', function() {
        ajax({
            service: '/cpservice/addmodel/delimage.json',
            data: {
...
addmodel.successAdd = function(id) {
    popup.open('popup-category-add', 'Báo đã thêm Model', template('/cp/tpl/model/successadd.tpl'), [
        {
            title: 'Lưu lại',
            style: 'btn-primary',
            fn: function() {
                ...
                popup.confirm('Bạn đã chắc chắn thêm xong Model?', function() {
                    ajaxSubmit({
                        service: '/cpservice/addmodel/successadd.json',
...
addmodel.showImages = function(id) {
    ajax({
        service: '/cpservice/global/model/getmodel.json',
        data: {id: id},
        done: function(resp) {
            if (resp.success) {
                popup.open('popup-model-show-images', 'Hình ảnh model', template('/cp/tpl/model/modelAddImagesEdit.tpl', {data: resp.data}), [
                    {
                        title: 'Đóng',
                        style: 'btn-info',
                        fn: function() {
...
addmodel.editProperties = function(id) {
    ajax({
                ...
                popup.open('popup-model-show-properties', 'Thuộc tính model', template('/cp/tpl/model/properties.tpl', resp), [
                    {
                        ...
                        fn: function() {
                            ...
                            ajax({
                                service: '/cpservice/addmodel/editproperties.json',
                                contentType: 'json',
...
addmodel.editDetail = function(id) {
    ajax({
        service: '/cpservice/global/model/getmodel.json',
        data: {id: id},
                ...
                popup.open('popup-edit-model', 'Sửa thông tin cơ bản model', template('/cp/tpl/model/add.tpl', result), [
                    {
                        ...
                        fn: function() {
                            ajaxSubmit({
                                service: '/cpservice/addmodel/edit.json',
                                id: 'model-add-form',
                                contentType: 'json',
                                done: function(resp) {
                                    if (resp.success) {
