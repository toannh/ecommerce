...
notification.config = function () {
    ...
    setTimeout(function () {
        $.each(notification.repData, function (i) {
            ...
            arr[2] = url;
            arr[3] = notification.repData[i].clickCount;
            data.push(arr);
        });
...
notification.click = function (id) {
    ajax({
        service: '/market/viewNotification.json',
        loading: false,
        data: {id : id},
        done: function (resp) {
            if (resp.success) {
                ...
                var data = [];
                $.each(notification.repData, function (i) {
                    if (notification.repData[i].id === id) {
                        notification.repData[i].clickCount = notification.repData[i].clickCount + 1;
                    }
                    var arr = [];
                    arr[0] = notification.repData[i].id;
                    arr[1] = notification.repData[i].title;
...
//            var url = baseUrl + '/tin-tuc/' + textUtils.createAlias(news[1]) + '-' + news[0] + '.html?rel=notification&code=' + news[0] + '';
            text += '<li><a href="' + url + '" target="_blank" onclick="notification.click(' + news[0] + ')">' + news[1] + '</a></li>';
        // onclick="notification.click(' + news[0] + ')"   (' + news[3] + ')
        });
