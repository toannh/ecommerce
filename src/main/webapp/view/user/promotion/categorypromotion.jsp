...
<div class="col-lg-9 col-md-9 col-sm-12 col-xs-12 slidebar-right">
    <ol class="breadcrumb">
        ...
        <li class="active">Giảm giá danh mục</li>
    </ol>
    <div class="tabs-intro"><a href="${baseUrl}/tin-tuc/huong-dan-tao-khuyen-mai-giam-gia-theo-danh-muc-975998236688.html" target="_blank"><span class="icon16-faq"></span>Hướng dẫn tạo khuyến mại giảm giá theo danh mục
        </a></div>     
    <h1 class="title-pages">Giảm giá danh mục</h1>
    <div class="tabs-content-user">
        <ul class="tab-title-content">
            <li class="active"><a href="${baseUrl}/user/categorypromotion.html">KM theo danh mục chợ</a></li>
            <li><a href="${baseUrl}/user/shopcategorypromotion.html">KM theo danh mục Shop</a></li>
        </ul>
        <div class="row row-reset">
                            ...
                            <c:forEach var="cate1" items="${category}" varStatus="">
                                <li class="clearfix" for="${cate1.id}">
                                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                        <tr>
                                            <td  width="47%" >
                                                <a onclick="promotion.getCateChilds('${cate1.id}', '');" >
            ...
            <div class="form-group text-center">
                <button class="btn btn-lg btn-danger" onclick="promotion.savePromotion('CATEGORY', 'DISCOUND');" >
    ...
    <div class="page-ouner clearfix">
        <ul class="pagination pull-right">
            <c:if test="${dataPage.pageIndex > 3}"><li><a href="${baseUrl}/user/categorypromotion.html?page=1" href="javascript:;"><<</a></li></c:if>
            <c:if test="${dataPage.pageIndex > 2}"><li><a href="${baseUrl}/user/categorypromotion.html?page=${dataPage.pageIndex}" ><</a></li></c:if>
            <c:if test="${dataPage.pageIndex > 3}"><li><a>...</a></li></c:if>
            <c:if test="${dataPage.pageIndex >= 3}"><li><a href="${baseUrl}/user/categorypromotion.html?page=${dataPage.pageIndex - 2}">${dataPage.pageIndex-2}</a></li></c:if>
            <c:if test="${dataPage.pageIndex >= 2}"><li><a href="${baseUrl}/user/categorypromotion.html?page=${dataPage.pageIndex - 1}" >${dataPage.pageIndex-1}</a></li></c:if>
            <c:if test="${dataPage.pageIndex >= 1}"><li><a href="${baseUrl}/user/categorypromotion.html?page=${dataPage.pageIndex}" >${dataPage.pageIndex}</a></li></c:if>
            <li class="active" ><a class="btn btn-primary">${dataPage.pageIndex + 1}</a>
            <c:if test="${dataPage.pageCount - dataPage.pageIndex > 1}"><li><a href="${baseUrl}/user/categorypromotion.html?page=${dataPage.pageIndex+2}" >${dataPage.pageIndex+2}</a></li></c:if>
            <c:if test="${dataPage.pageCount - dataPage.pageIndex > 2}"><li><a href="${baseUrl}/user/categorypromotion.html?page=${dataPage.pageIndex+3}" >${dataPage.pageIndex+3}</a></li></c:if>
            <c:if test="${dataPage.pageCount - dataPage.pageIndex > 3}"><li><a >...</a></c:if>
            <c:if test="${dataPage.pageCount - dataPage.pageIndex > 2}"><li><a href="${baseUrl}/user/categorypromotion.html?page=${dataPage.pageIndex+2}" >></a></li></c:if>
            <c:if test="${dataPage.pageCount - dataPage.pageIndex > 2}"><li><a href="${baseUrl}/user/categorypromotion.html?page=${dataPage.pageCount}" >>></a></li></c:if>
            </ul>
        </div>
    </div>
