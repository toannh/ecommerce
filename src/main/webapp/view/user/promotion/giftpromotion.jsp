...
<div class="col-lg-9 col-md-9 col-sm-12 col-xs-12 slidebar-right">
    ...
    <div class="tabs-intro"><a href="${baseUrl}/tin-tuc/huong-dan-tao-khuyen-mai-qua-tang-494772223758.html" target="_blank"><span class="icon16-faq"></span>Hướng dẫn tạo khuyến mại quà tặng
        </a></div>    
    <h1 class="title-pages">Khuyến mại quà tặng</h1>
    <div class="tabs-content-user">
        <div class="row row-reset">
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="clearfix">
                    <h6 class="pull-left" ><strong>Nội dung khuyến mại:</strong> </h6>
                    <span class="pull-right small mgt-10">Tối đa 100 ký tự - <span class="clr-org"><strong>Còn <span class="countresults" >100/100</span></strong></span> ký tự</span>
                </div>
                <textarea maxlength="100" name="promotionGift" class="form-control" rows="5" onkeyup="promotion.countPromotion();" placeholder="Ví dụ: Mua sản phẩm tặng kèm lịch năm mới." ></textarea>
                <p class="text-left mgt-10 clearfix">Chiến dịch giảm giá sẽ được quảng bá rộng rãi trên ChợĐiệnTử. <a href="#" data-toggle="modal" data-target="#ModalNormal">Xem ví dụ</a> </p>
            </div>
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <h6 class="mgt-15">
                        <strong>Đối tượng áp dụng (danh mục hoặc sản phẩm):</strong> 
                        <div class="text-danger errorMessages" for="categories"  ></div>
                    </h6>
                    <div class="border-box-cod promotion-content-gift">
                        ...
                        <div class="col-lg-7 col-md-7 col-sm-12 col-xs-12 reset-padding-right">
                            <div class="title-config-shop">
                                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                    <tbody><tr>
                                            ...
                                            <td><div class="text-center">Nội dung khuyến mại</div></td>
                                        </tr>
                                    </tbody></table>
                            </div>
                            ...
                            <div class="promotion-footer">
                                ...
                                <ul class="pagination pull-left" style="margin: 0px 0px 10px 0px;" >
            ...
            <div class="form-group text-center mgt-15">
                <button class="btn btn-lg btn-danger" onclick="promotion.savePromotion('ITEM', 'GIFT');" >
            ...
            <div class="table-responsive list-box-product mgt-10">
                <table class="table" width="100%">
                    ...
                    <c:forEach var="item" items="${dataPage.data}">
                        <tr>
                            <td><div class="text-left " onclick="promotion.viewDetail('${item.id}')"><a href="javascript:;">${item.name}</a></div></td>
                            <td>
        ...
        <div class="page-ouner clearfix">
            <ul class="pagination pull-right">
                <c:if test="${dataPage.pageIndex > 3}"><li><a href="${baseUrl}/user/giftpromotion.html?page=1" href="javascript:;"><<</a></li></c:if>
                <c:if test="${dataPage.pageIndex > 2}"><li><a href="${baseUrl}/user/giftpromotion.html?page=${dataPage.pageIndex}" ><</a></li></c:if>
                <c:if test="${dataPage.pageIndex > 3}"><li><a>...</a></li></c:if>
                <c:if test="${dataPage.pageIndex >= 3}"><li><a href="${baseUrl}/user/giftpromotion.html?page=${dataPage.pageIndex - 2}">${dataPage.pageIndex-2}</a></li></c:if>
                <c:if test="${dataPage.pageIndex >= 2}"><li><a href="${baseUrl}/user/giftpromotion.html?page=${dataPage.pageIndex - 1}" >${dataPage.pageIndex-1}</a></li></c:if>
                <c:if test="${dataPage.pageIndex >= 1}"><li><a href="${baseUrl}/user/giftpromotion.html?page=${dataPage.pageIndex}" >${dataPage.pageIndex}</a></li></c:if>
                <li class="active" ><a class="btn btn-primary">${dataPage.pageIndex + 1}</a>
                <c:if test="${dataPage.pageCount - dataPage.pageIndex > 1}"><li><a href="${baseUrl}/user/giftpromotion.html?page=${dataPage.pageIndex+2}" >${dataPage.pageIndex+2}</a></li></c:if>
                <c:if test="${dataPage.pageCount - dataPage.pageIndex > 2}"><li><a href="${baseUrl}/user/giftpromotion.html?page=${dataPage.pageIndex+3}" >${dataPage.pageIndex+3}</a></li></c:if>
                <c:if test="${dataPage.pageCount - dataPage.pageIndex > 3}"><li><a >...</a></c:if>
                <c:if test="${dataPage.pageCount - dataPage.pageIndex > 2}"><li><a href="${baseUrl}/user/giftpromotion.html?page=${dataPage.pageIndex+2}" >></a></li></c:if>
                <c:if test="${dataPage.pageCount - dataPage.pageIndex > 2}"><li><a href="${baseUrl}/user/giftpromotion.html?page=${dataPage.pageCount}" >>></a></li></c:if>
                </ul>
            </div>
        </div>
    </div>
